...
- **CI/CD の関心分離 (GitHub Actions)**: ワークフローは役割ごとに分割・整理されています。
  ...
  - `deploy.yml`: テンプレート利用者が本番環境へ純粋にデプロイするための雛形です。Cloudflare Pages には GitHub リポジトリと連携した **自動デプロイ標準機能** が備わっているため、GitHub Actions 経由のデプロイ (`deploy.yml`) は基本的にオプション機能（手動でデプロイタイミングを制御したい場合など）となります。
