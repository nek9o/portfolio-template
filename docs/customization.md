...
- **配色**: `uno.config.ts` 内の `theme.colors` または `src/app.css` の CSS 変数を編集することで、サイト全体のトーンを変更できます。
- **フォント**: `uno.config.ts` の `theme.fontFamily` または `src/routes/+page.svelte` の Google Fonts URL を変更してください。
